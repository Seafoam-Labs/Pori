...
public static class MountOptionsDialog
{
    ...
    private static void AddField(Box box, string fieldName, string value)
    {
     ...
     public static Task<MountDialogResult?> EditMountOptionsAsync(Overlay parentOverlay, MountCatInfo model)
    {
        var tcs = new TaskCompletionSource<MountDialogResult?>();
        ...
        var heading = Label.New($"Edit {model.UnitName}");
        heading.AddCssClass("title-4");
        heading.SetXalign(0);
        content.Append(heading);
        ...
        var mountEntry = Entry.New();
        mountEntry.SetText(model.Where);
        mountEntry.SetHexpand(true);
        content.Append(mountEntry);
        ...
        var descEntry = Entry.New();
        descEntry.SetPlaceholderText("e.g. Games drive, Backup disk");
        descEntry.SetText(model.Description);
        descEntry.SetHexpand(true);
        content.Append(descEntry);
        ...
        var currentOptions = model.Options?.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries) ?? Array.Empty<string>();
        var recommended = MountOptionBuilder.GetRecommendedOptions(model.Type);
        var additional = currentOptions.Where(o => !recommended.Contains(o)).ToList();

        var optionsBox = MountOptionBuilder.BuildOptionsBox(model.Type, out var mountCheckButtons, currentOptions);
        if (optionsBox != null)
            content.Append(optionsBox);
        ...
        var optionsEntry = Entry.New();
        optionsEntry.SetPlaceholderText("e.g. nofail");
        optionsEntry.SetText(string.Join(",", additional));
        optionsEntry.SetHexpand(true);
        content.Append(optionsEntry);
        ...
        var confirmButton = Button.NewWithLabel("Edit");
        confirmButton.AddCssClass("suggested-action");
        confirmButton.OnClicked += (_, _) =>
        {
            var mountPoint = mountEntry.GetText();
